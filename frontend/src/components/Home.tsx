...
const featuredItems: Item[] = [
  { id: 1, image: "/HomePage/purse.jpg", title: "Elegant Purse", condition: "Barely used, perfect condition"},
  { id: 2, image: "/HomePage/laptop.jpg", title: "Gaming Laptop", condition: "Recently serviced"},
  { id: 3, image: "/HomePage/clothes.jpg", title: "Designer Outfit", condition: "Brand new with tags" },
];
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-blue-50 to-indigo-50 text-slate-900 flex flex-col items-center px-6">
      ...
      <div className="text-center max-w-3xl mt-24 mb-24">
        ...
        <p className="mt-6 text-xl text-slate-600">
          ...
          <span className="text-blue-600 font-semibold"> Sustainable. Smart. Affordable.</span>
        </p>
      ...
      <div className="max-w-6xl w-full text-center">
        <h2 className="text-4xl font-bold mb-4">Built for Smart Sustainable Commerce</h2>
        <p className="text-slate-600 mb-12 text-lg">
          Join thousands of users making smarter buying decisions while reducing waste.
        </p>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">

          <TrustCard title="Sustainable Shopping" icon="♻" text="Extend product lifecycles and reduce environmental waste." />
          <TrustCard title="Affordable Pricing" icon="₹" text="Save up to 70% compared to buying new products." />
          <TrustCard title="Verified Community" icon="🛡" text="Trusted sellers, ratings, and secure transactions." />
      ...
      <div className="mt-16 w-full max-w-7xl">
        <h2 className="text-4xl font-bold text-center mb-14">Featured Listings</h2>
...
const TrustCard = ({ title, icon, text }: { title: string; icon: string; text: string }) => (
...
const ItemCard: React.FC<ItemCardProps> = ({ item }) => (
  <div className="group bg-white border border-slate-200 rounded-2xl p-6 shadow-md hover:shadow-xl transition transform hover:-translate-y-3">
    ...
    <div className="overflow-hidden rounded-xl">
      <img src={item.image} alt={item.title} className="w-full h-64 object-cover group-hover:scale-105 transition duration-500" />
    </div>
